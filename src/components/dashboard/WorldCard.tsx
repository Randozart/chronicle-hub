...
import { useTheme } from '@/providers/ThemeProvider'; // <--- Import Theme Hook
...
export default function WorldCard({ w, isOwner, isGuest = false }: WorldCardProps) {
    ...
    // 1. Get the User's Global Theme preference
    ...
    // Check if there are multiple creators
    ...
    // 2. Helper to determine the correct font based on Global Theme
    // This mirrors the logic in theme.css to ensure consistency
    const getGlobalFontStack = () => {
        switch (globalTheme) {
            case 'classic': 
            case 'classic-light':
            case 'soft-light': // "Literary" feel
                return 'var(--font-garamond), "Georgia", "Times New Roman", serif';
            
            case 'terminal': // "Hacker" feel
    ...
    return (
        <div 
            ...
            data-theme={worldTheme} // This applies the WORLD style to the card
            style={{ 
        ...
        >
            ...
            <div 
                ...
                style={{
                    ...
                    // FIX: Override the World Font with the User's Preferred App Font
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                            ...
                            <div>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                    ...
                                    <span style={{ fontSize: '0.7rem', background: '#3498db', color: 'white', padding: '2px 6px', borderRadius: '4px' }}>OWNER</span>
                                </div>
            ...
            <div style={{ width: '100%', aspectRatio: '16/9', background: '#000', position: 'relative', borderBottom: '1px solid var(--border-color)', overflow: 'hidden' }}>
                ...
                {isOwner && w.ownerId && w.currentUserId && w.ownerId !== w.currentUserId && (
                    <div style={{ position: 'absolute', top: 10, right: 10, background: '#f1c40f', color: 'black', padding: '4px 8px', borderRadius: '4px', fontSize: '0.65rem', fontWeight: 'bold', boxShadow: '0 2px 4px rgba(0,0,0,0.3)', zIndex: 20 }}>COLLABORATOR</div>
                )}
                ...
                <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: '34px', background: 'linear-gradient(to top, rgba(0,0,0,0.95), rgba(0,0,0,0))', display: 'flex', alignItems: 'center', padding: '0 12px', gap: '8px', zIndex: 10 }}>
                    ...
                    {(hasTeam || hasAttributions) && (
                        <button 
                            ...
                            style={{ background: 'rgba(52, 152, 219, 0.25)', border: '1px solid rgba(52, 152, 219, 0.5)', color: '#3498db', borderRadius: '3px', fontSize: '0.85rem', padding: '2px 8px', cursor: 'pointer', fontWeight: 'bold', textTransform: 'uppercase', backdropFilter: 'blur(2px)', transition: 'all 0.2s' }}
                            title="View Creators"
                        >
                            Creators
                        </button>
                    ...
                    {hasAI && (
                        <button 
                            ...
                            style={{ background: 'rgba(243, 156, 18, 0.25)', border: '1px solid rgba(243, 156, 18, 0.5)', color: '#f39c12', borderRadius: '3px', fontSize: '0.85rem', padding: '2px 8px', cursor: 'pointer', fontWeight: 'bold', textTransform: 'uppercase', backdropFilter: 'blur(2px)', transition: 'all 0.2s' }}
            ...
            <div style={{ padding: '1.25rem', flex: 1, display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ marginTop: 'auto', display: 'flex', gap: '10px' }}>
                    <Link href={`/play/${w.worldId}`} className="continue-button" style={{ flex: 1, textDecoration: 'none', padding: '0.6rem', textAlign: 'center', borderRadius: '4px', fontSize: '0.9rem', fontWeight: '500' }}>Play</Link>
                    {isOwner && (
