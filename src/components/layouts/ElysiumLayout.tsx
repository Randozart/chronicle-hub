...
export default function ElysiumLayout(props: LayoutProps) {
    ...
    const handleMouseMove = (e: React.MouseEvent) => {
        if (!parallaxEnabled) return; // Skip calculation if disabled
    ...
    return (
        <div 
            ...
            style={{ height: '100vh', display: 'flex', overflow: 'hidden', position: 'relative', color: '#eee', background: '#000' }}
        >
            
            {/* --- PARALLAX BACKGROUND LAYER --- */}
            <div style={{ 
                position: 'absolute', 
                // Negative inset allows the image to move without showing white space
                top: '-50px', left: '-50px', right: '-50px', bottom: '-50px', 
                zIndex: 0,
                transition: 'transform 0.1s ease-out',
                transform: `translate3d(${-moveX}px, ${-moveY}px, 0)` // Will remain 0,0 if disabled
            }}>
                <img 
                    src={bgSrc}
                    alt=""
                    style={{ 
                        ...
                        objectFit: 'cover', 
                        filter: 'brightness(0.6)' // Dim slightly for readability
                    }}
            ...
            <div style={{ 
                ...
                <div style={{ padding: '2rem', paddingBottom: '1rem', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                    <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center' }}>
                        <button onClick={() => setActiveTab('story')} style={{ background: 'none', border: 'none', color: activeTab === 'story' ? '#fff' : '#666', fontWeight: 'bold', cursor: 'pointer', textTransform: 'uppercase', fontSize: '0.8rem', letterSpacing: '1px', borderBottom: activeTab === 'story' ? '2px solid #fff' : '2px solid transparent', paddingBottom: '5px' }}>Story</button>
                        <button onClick={() => setActiveTab('possessions')} style={{ background: 'none', border: 'none', color: activeTab === 'possessions' ? '#fff' : '#666', fontWeight: 'bold', cursor: 'pointer', textTransform: 'uppercase', fontSize: '0.8rem', letterSpacing: '1px', borderBottom: activeTab === 'possessions' ? '2px solid #fff' : '2px solid transparent', paddingBottom: '5px' }}>Items</button>
                        <button onClick={() => setActiveTab('profile')} style={{ background: 'none', border: 'none', color: activeTab === 'profile' ? '#fff' : '#666', fontWeight: 'bold', cursor: 'pointer', textTransform: 'uppercase', fontSize: '0.8rem', letterSpacing: '1px', borderBottom: activeTab === 'profile' ? '2px solid #fff' : '2px solid transparent', paddingBottom: '5px' }}>Stats</button>
                    </div>
                </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', position: 'relative', zIndex: 5 }}>
                <div style={{ maxWidth: '900px', margin: '0 auto', padding: '0 2rem 4rem 2rem', minHeight: '100%' }}>
                    {activeTab === 'story' ? renderContent() : (
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', color: 'rgba(255,255,255,0.5)', fontStyle: 'italic', fontSize: '1.2rem' }}>
