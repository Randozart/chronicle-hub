    ...
    return (
        <div className="wallet-container" style={{ 
            ...
            {currencyIds.map(rawId => {
                // Handle cases where user typed "$gold" or "gold"
                const qid = rawId.replace('$', '').trim();
                const def = qualityDefs[qid];
                const state = qualities[qid];
                
                // If quality doesn't exist yet (0), show 0 if valid def, else hide
                if (!def) return null;
                    ...
                    <div key={qid} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }} title={def.description}>
                        <div style={{ width: '24px', height: '24px', flexShrink: 0 }}>
                            <GameImage 
                                ...
                                type="icon" 
                                className="option-image" // Reusing existing class for consistent radius/border
                            />
                        </div>
