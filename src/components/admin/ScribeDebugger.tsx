...
export default function ScribeDebugger({ onUpdate }: Props) {
    ...
    // Debounce the broadcasting to prevent spamming updates
    useEffect(() => {
        const timer = setTimeout(() => {
            ...
            rows.forEach(row => {
                ...
                // 1. Create Mock Definition based on detected type
                mockDefs[key] = {
                    id: key,
                    name: key,
                    type: type,
                ...
                // 2. Create Mock State based on detected type
                if (isNumeric) {
                    mockQualities[key] = {
                        qualityId: key,
                        type: type,
                        level: numericValue, 
                        // For Pyramidal, CP needs to be calculated for some logic checks
                    ...
                    mockQualities[key] = {
                        ...
                        type: type,
                        stringValue: String(row.value),
                        level: 0, // String qualities have a level of 0
                        changePoints: 0,
                    } as any;
                }
            });
