...
export default function ProbabilityChart({ operator, target, margin, minCap, maxCap, pivot }: Props) {
    ...
    return (
        <div style={{ 
            ...
            position: 'relative',
            width: '100%',
            aspectRatio: '2/1' // Maintains visual consistency regardless of container size
        }}>
...
function calculateChance(skill: number, op: string, target: number, margin: number, min: number, max: number, pivot: number) {
    ...
    // Precise (Bell Curve-ish)
    // Matches BOTH '==' (Legacy) and '><' (New)
    else if (op === '==' || op === '><') {
        const dist = Math.abs(skill - target);
        if (dist >= margin) {
            chance = 0;
        } else {
            // Linear falloff from Target (100% or Pivot?)
            // Usually Precise means "Exact match is best".
            // Standard implementation: Exact = 100%, +/- Margin = 0%
            chance = 1.0 - (dist / margin);
            
            // NOTE: If you want 'Pivot' to scale the peak height of a Precise check, 
            // you would multiply here: chance = (1.0 - (dist / margin)) * (pivot / 100);
            // But usually Precise implies aiming for perfection = 100%. 
            // Leaving as linear 1.0 for now unless requested.
        }
    }
    // Avoid (Inverted Bell Curve)
    // Matches BOTH '!=' (Legacy) and '<>' (New)
    else if (op === '!=' || op === '<>') {
        const dist = Math.abs(skill - target);
        if (dist >= margin) {
