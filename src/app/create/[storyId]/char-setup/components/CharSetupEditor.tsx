...
export default function CharSetupEditor({
    ...
    const presetDefs = [
        { label: 'Simple Name',      description: "Single text field for the player's character name.",                   has: hasRule('$player_name') && !hasRule('$first_name'), onAdd: addSimpleIdentity, onRemove: removeSimpleIdentity },
        { label: 'Complex Name',     description: 'First + last name fields combined into $player_name via a modal.',      has: hasRule('$first_name'),                             onAdd: addComplexIdentity, onRemove: removeComplexIdentity },
        { label: 'Simple Stats',     description: 'Body / Mind / Spirit qualities each set to 10 at start.',              has: hasRule('$body'),                                   onAdd: addSimpleStats, onRemove: removeSimpleStats         },
        { label: 'Class System',     description: 'Warrior / Mage / Rogue picker with derived attributes & starter gear.',has: hasRule('$class'),                                  onAdd: addClassSystem, onRemove: removeClassSystem         },
        { label: 'Pronoun System',   description: 'He / She / They selector with custom pronoun fallback fields.',        has: hasRule('$pronouns'),                               onAdd: addPronounSystem, onRemove: removePronounSystem     },
        { label: 'Starting Location',description: 'Lets players pick their starting location from a defined list.',       has: hasRule('$location'),                               onAdd: addVariableLocation, onRemove: removeVariableLocation },
    ];
    ...
    return (
        <div>
            ...
            <div style={{ background: 'var(--tool-bg-header)', border: '1px solid var(--tool-border)', borderRadius: '6px', padding: '1rem', marginBottom: '1.5rem' }}>
                ...
                <p className="special-desc" style={{ marginTop: '0.5rem', marginBottom: 0 }}>
                    If enabled, players click &quot;Start New Game&quot; and bypass the form entirely.{' '}
                    <strong>Static/Calculated</strong> rules still run.{' '}
                    <strong>Interactive</strong> fields (inputs, selects) will default to empty since the player cannot interact with them.
                </p>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
                ...
                <span style={{ fontSize: '0.75rem', color: 'var(--tool-text-dim)' }}>{sortedKeys.length} field{sortedKeys.length !== 1 ? 's' : ''}</span>
            </div>
            ...
            {/* Add rule */}
            <div style={{ marginTop: '1.5rem', paddingTop: '1rem', borderTop: '1px solid var(--tool-border)', display: 'flex', gap: '10px' }}>
                <input
                    value={newKey}
                    ...
                    style={{ flex: 1 }}
                />
                <button onClick={handleAdd} className="save-btn" style={{ width: 'auto', padding: '0.5rem 1.5rem', height: 'fit-content' }}>
                    Add Rule
                </button>
            </div>
        </div>
...
function RuleRow({ ruleKey, rule, isDragged, showSkippedWarning, imageLibrary, onUpdate, onDelete, onDragStart, onDragOver, onDrop }: RuleRowProps) {
    const isSelectType = SELECT_TYPES.includes(rule.type);
    const accent = rule.type === 'header' ? 'var(--tool-accent-mauve)' : 'var(--success-color)';
    ...
    return (
        ...
        >
            ...
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center', marginBottom: '0.65rem' }}>
                <span style={{ opacity: 0.35, fontSize: '0.9rem', cursor: 'grab' }}>☰</span>
                <span style={{ fontFamily: 'monospace', color: accent, flex: 1, fontSize: '0.875rem', userSelect: 'none' }}>
                    {ruleKey}
                </span>
                {badges.map(b => (
                    <span
                        ...
                        style={{
                            ...
                            border: `1px solid ${b.color}`, color: b.color, lineHeight: '1.5',
                        }}
                    >
                        {b.label}
                    </span>
                ...
                <button
                    ...
                    title="Delete rule"
                    style={{ color: 'var(--danger-color)', background: 'none', border: 'none', cursor: 'pointer', padding: '2px 4px', lineHeight: 1 }}
                >
                    ✕
                </button>
            </div>
            ...
            {rule.type === 'header' ? (
                ...
                <div>
                    ...
                    <span className="property-hint" style={{ marginLeft: 0 }}>
                        {rule.type === 'static' ? 'Value / ScribeScript expression' : 'Default text (player can edit)'}
                    </span>
                </div>
            )}
            ...
            <details style={{ marginTop: '0.5rem' }}>
                <summary style={{ cursor: 'pointer', fontSize: '0.72rem', color: 'var(--tool-text-dim)', userSelect: 'none', paddingTop: '0.2rem' }}>
                    Advanced
                </summary>
                <div style={{ marginTop: '0.5rem', paddingTop: '0.5rem', borderTop: '1px dashed var(--tool-border)' }}>
                    ...
                    <div style={{ marginBottom: '0.5rem' }}>
                        <input
                            ...
                            onChange={e => onUpdate(ruleKey, 'visible_if', e.target.value)}
                            className="form-input"
                            placeholder="e.g. $class == Rogue"
                        />
                        <span className="property-hint" style={{ marginLeft: 0 }}>Visible If (ScribeScript condition)</span>
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', fontSize: '0.8rem' }}>
                        ...
                        {rule.type !== 'header' && (
                            <label className="toggle-label" title="If this field sits inside a Modal section, also show it on the main card">
...
function OptionBuilder({ type, rawValue, onChange, imageLibrary }: OptionBuilderProps) {
        ...
        <div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(80px, 1fr))', gap: '10px' }}>
                            {Object.values(imageLibrary).map(img => (
                                <div
                                    key={img.id}
                                    onClick={() => {
                                        // For image_select, auto-fill label = image id; for labeled_image_select, leave label as-is
                                        const next = rows.map((r, i) => {
                                            if (i !== pickingForIdx) return r;
                                            return type === 'image_select'
                                                ? { value: img.id, label: img.id }
                                                : { ...r, value: img.id };
                                        });
                                        onChange(serializePipeRows(next));
                    ...
                    <div key={idx} style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                        ...
                        <div style={{ flex: 1, position: 'relative' }}>
                            <input
                                ...
                                className="form-input"
                                placeholder={isImageType ? 'image_id' : 'value'}
                                style={{ fontSize: '0.8rem', paddingRight: isImageType ? '52px' : undefined }}
                            />
                            {isImageType && imageLibrary[row.value] && (
                                <img
                                    src={imageLibrary[row.value].url}
                                    alt=""
                                    style={{ position: 'absolute', right: 34, top: '50%', transform: 'translateY(-50%)', height: '18px', width: '18px', objectFit: 'cover', borderRadius: '2px', pointerEvents: 'none' }}
                                />
                            )}
                            {isImageType && (
                                <button
                                    onClick={() => setPickingForIdx(idx)}
                                    style={{ position: 'absolute', right: 4, top: '50%', transform: 'translateY(-50%)', padding: '1px 5px', fontSize: '0.65rem', background: 'var(--tool-bg-dark)', border: '1px solid var(--tool-border)', color: 'var(--tool-text-main)', borderRadius: '3px', cursor: 'pointer' }}
                                >
                                    Pick
                                </button>
                            )}
                        ...
                        <button
                            ...
                            style={{ color: 'var(--danger-color)', background: 'none', border: 'none', cursor: 'pointer', fontSize: '0.8rem', padding: '0 4px', flexShrink: 0 }}
                        >
                            ✕
                        </button>
                    </div>
            ...
            <details style={{ marginTop: '6px' }}>
                ...
                <textarea
                    ...
                    rows={2}
                    style={{ fontFamily: 'monospace', fontSize: '0.72rem', marginTop: '4px' }}
                />
            </details>
        </div>
    );
        ...
        <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '6px' }}>
                ...
                {has ? (
                    <button
                        onClick={onRemove}
                        style={{ fontSize: '0.68rem', padding: '2px 8px', background: 'transparent', color: 'var(--danger-color)', border: '1px solid var(--danger-color)', borderRadius: '3px', cursor: 'pointer', flexShrink: 0 }}
                    >
                        Remove
                    </button>
                ) : (
                    <button
                        onClick={onAdd}
                        style={{ fontSize: '0.68rem', padding: '2px 8px', background: 'transparent', color: 'var(--tool-accent)', border: '1px solid var(--tool-accent)', borderRadius: '3px', cursor: 'pointer', flexShrink: 0 }}
                    >
                        Add
                    </button>
                )}
